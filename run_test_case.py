...
def send_mail(report):
    ...
    smtp.login('linxiaoqiang@zhankoo.com', 'Lxq123456')
    smtp.sendmail('linxiaoqiang@zhankoo.com', 'linxiaoqiang@zhankoo.com', msg.as_string())
    smtp.quit()
...
if __name__ == '__main__':
    ...
    fp = open(filename, 'wb')
    runner = HTMLTestRunner(stream=fp, title='展酷网自动化测试报告', description='')
    discover = unittest.defaultTestLoader.discover(base + '\\test_case',pattern='test*.py')
    runner.run(discover)
    fp.close()
